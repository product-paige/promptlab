...
export default function Layout({ children, currentPageName }) {
  ...
  return (
    <div className="min-h-screen bg-[#F7F7F7] p-[10px]">
        ...
        <nav className="flex-1 px-3 space-y-1">
          {navItems.map((item) => (
            <Link
              ...
              style={isActive(item.page) ? { background: '#F7F7F7' } : {}}
              onMouseEnter={(e) => (e.currentTarget.style.background = '#F7F7F7')}
              onMouseLeave={(e) => isActive(item.page) ? (e.currentTarget.style.background = '#F7F7F7') : (e.currentTarget.style.background = '')}
            >
              <div className="flex items-center gap-3">
      ...
      {/* Main Content */}
      <main className="pt-16 lg:pt-0 bg-white rounded-lg" style={{ margin: '10px', marginTop: '10px', marginLeft: 'calc(256px + 10px)', minHeight: 'calc(100vh - 20px)' }}>
